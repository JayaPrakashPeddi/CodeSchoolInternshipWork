...
class AuthControllers
{
    ...
    public function setOptAndPhone($phone, $rememberMe)
    {
        ...
        $this->db->query("DELETE FROM otps WHERE phone_number=:phone")->execute([":phone" => $phone]);
    ...
    {
        $this->db->query("DELETE FROM otps WHERE CURRENT_TIMESTAMP>otp_expires_at");
        return;
    }
    ...
    public function verifyOtp($phone, $otp)
    {
        ...
        $is_verified = $this->db->query("SELECT 1 FROM otps WHERE phone_number=:phone AND otp=:otp AND otp_expires_at>CURRENT_TIMESTAMP")->get([":phone" => $phone, ":otp" => $otp]);
        if (!$is_verified) {
    ...
    public function login($phone, $rememberMe)
    {
        ...
        $data['token'] = $token;
        $this->db->query("DELETE FROM user_tokens WHERE user_id=:id")->execute([":id"=>$userId]);
        if ($rememberMe) {
    ...
    public function validateToken($token){
        $userId = $this->db->query("SELECT user_id FROM user_tokens WHERE token=:token AND expires_at>CURRENT_TIMESTAMP")->get([":token"=>$token]);
        if($userId){
